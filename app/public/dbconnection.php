...
class DbConnection
{
    ...
    function lookup_referral(string $accessionnummer): ?Result
    {
        ...
        $stmt = db2_prepare(
            $connection,
            'select RECEIVER_ID,RECEIVER_ID_TYPE,CCRECEIVER_ID,CCRECEIVER_ID_TYPE from ' .
                DbConnection::table .
    ...
    function save_referral(
     ...
     $stmt = db2_prepare(
         ...
         set RECEIVER_ID = ?, RECEIVER_ID_TYPE = ?, CCRECEIVER_ID = ?, CCRECEIVER_ID_TYPE = ? where ACCESSION_NUMBER = ?'
     );
     ...
     $clean_input = $this->evaluate_input([$receiver, $receiver_type, $cc_receiver, $cc_receiver_type, $accessionnummer]);
